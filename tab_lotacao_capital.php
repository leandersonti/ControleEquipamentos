...
<?php
$sql = "SELECT COUNT(*) AS qtdd FROM equipamento,e_lotado,ligacao WHERE num_serie=e_num_serie and prot_lotacao=protocolo and lot_status=0";
...
<div id="tabelinha" class="table">
	...
	{
		$query = "SELECT * FROM ligacao,e_lotado,equipamento WHERE e_num_serie=num_serie and prot_lotacao=protocolo and lot_status=0";
		...
		if ($_POST['dpto']!="")
		{
			$query .= " and dpto='".$_POST['dpto']."'";
		}
		if ($_POST['dpto']=="" && $_POST['responsavel']=="" && $_POST['equipamento']=="")
		{
			$query = "SELECT * FROM ligacao,e_lotado,equipamento  WHERE num_serie = e_num_serie and prot_lotacao = protocolo and lot_status=0";
		}
		$query .=  " ORDER BY data_lotacao";
	}else{
		$query = "SELECT * FROM ligacao,e_lotado,equipamento  WHERE num_serie = e_num_serie and prot_lotacao = protocolo and lot_status=0 ORDER BY data_lotacao";
	}
	...
	while ($fo = mysqli_fetch_object($result))
	{
		...
		<div class="linha cor<?php echo $cor; ?>">
			...
			<div onclick="devolver('<?php echo $i; ?>')" class='coluna_icone link'>
