...
<body>
	<div style="width:80%; margin:100px auto" class="panel panel-defalt">
			...
			<div class="form-row">
				<div class="form-group col-md-3">
					<select name="equipamento" class="form-control">
						...
						<?php
						$query = "SELECT * FROM ligacao,equipamento,e_lotado WHERE e_num_serie=num_serie and prot_lotacao=protocolo and lot_status=0 GROUP BY tipo";
				...
				<div class="form-group col-md-3">
					<select name="responsavel" class="form-control">
						...
						<?php
						$query = "SELECT * FROM ligacao,equipamento,e_lotado WHERE e_num_serie=num_serie and prot_lotacao=protocolo and lot_status=0 GROUP BY responsavel";
						$result = mysqli_query($conn,$query);
						while ($rs = mysqli_fetch_object($result))
						{
				...
				<div class="form-group col-md-3">
					<select name="dpto" class="form-control">
						...
						<?php
						$query = "SELECT * FROM ligacao,equipamento,e_lotado WHERE e_num_serie=num_serie and prot_lotacao=protocolo and lot_status=0 GROUP BY dpto";
						$result = mysqli_query($conn,$query);
						while ($rs = mysqli_fetch_object($result))
						{
							echo "<option value='".$rs->dpto."'>".$rs->dpto."</option>";
						}
						?>
					</select>
				</div>
		...
		<?php
		$query = "SELECT COUNT(*) as qtdd FROM equipamento,e_lotado,ligacao WHERE num_serie=e_num_serie and protocolo=prot_lotacao and lot_status=0";
		$result = mysqli_query($conn,$query);
